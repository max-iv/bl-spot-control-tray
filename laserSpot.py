...
def getReading(camera,cameraSettings):
    ...
    with picamera.array.PiRGBArray(camera) as output:
        ...
        deltat = round(time.process_time() - tstart,2)
        if maxRawSignal < cameraSettings['minIntensity']:
          ...
          readingString = readingString  + ',"acqTime":'       + str(deltat) 
        ...
        cut = round(maxRawSignal * cameraSettings['noiseFloorCut'])
        for x in crop:
          icol = -cropPix2
          for y in x:
            if output.array[row + irow][col + icol][0] > cut:
                crop[irow + cropPix2][icol + cropPix2] =  output.array[row + irow][col + icol][0]
            if output.array[row + irow][col + icol][1] > cut:
                crop[irow + cropPix2][icol + cropPix2] =  crop[irow + cropPix2][icol + cropPix2] + output.array[row + irow][col + icol][1]   
            if output.array[row + irow][col + icol][2] > cut:
